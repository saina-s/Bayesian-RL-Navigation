...
* MAP model-posterior ties and MAP action-value ties are controlled separately.
  The reproduction default is: first model among posterior ties, then random
  selection among exactly tied greedy actions. This is the convention supported
  by the Figure 8/9 exact-Q sensitivity diagnostics.
...
python final_bayesian_navigation.py --figure 8 --mode evaluate-all --map-tie-rule first --map-action-tie-rule random --trials 1000 --eval-horizon 50
...
python final_bayesian_navigation.py --figure 9 --mode evaluate-all --map-tie-rule first --map-action-tie-rule random --trials 1000 --eval-horizon 50
...
def build_exact_comparator_bank(
    scenario: NavigationScenario,
    gamma: float = 0.95,
    tolerance: float = 1e-12,
    max_iterations: int = 10000,
) -> ComparatorBank:
    """Compute exact model-specific Q* tables by value iteration.

    This is used only for the MAP comparator when ``--map-q-backend exact``.
    It does not train or modify any checkpoint.  Baseline and Active Learning
    continue to use the supplied comparator bank.
    """
    environment = MazeEnvironment(scenario.config)
    ...
    for model_index, model in enumerate(scenario.models):
        mdp = ModelSpecificMDP(
            environment, model, scenario.config.unknown_cells
        )
        ...
        for iteration in range(1, max_iterations + 1):
            for state_index in range(n_states):
                for action_index in range(n_actions):
                    q_values[state_index, action_index] = sum(
                        probability
                        * (reward + gamma * values[next_index])
                        for next_index, probability, reward
                        in kernel[state_index][action_index]
                    )
            next_values = np.max(q_values, axis=1)
            residual = float(np.max(np.abs(next_values - values)))
            values = next_values
            if residual <= tolerance:
                break
        else:
            raise RuntimeError(
                f"Value iteration did not converge for model {model_index}; "
                f"residual={residual}"
            )

        # Synchronize Q with the converged V.
        for state_index in range(n_states):
            for action_index in range(n_actions):
                q_values[state_index, action_index] = sum(
                    probability * (reward + gamma * values[next_index])
                    for next_index, probability, reward
                    in kernel[state_index][action_index]
                )

        q_tables.append(q_values.copy())
        print(
            f"exact_MAP_model={model_index + 1}/{len(scenario.models)} "
            f"model={[cell.value for cell in model]} "
            f"vi_iterations={iteration} residual={residual:.3e}",
            flush=True,
        )

    if reference_states is None:
        raise RuntimeError("No environment models available for exact MAP.")

    stacked = np.stack(q_tables)
    return ComparatorBank(
        models=scenario.models,
        states=reference_states,
        q_values=stacked,
        visit_counts=np.zeros_like(stacked, dtype=np.int64),
        model_seeds=np.full(len(scenario.models), -1, dtype=np.int64),
        config=ComparatorConfig(
            episodes=0, horizon=0, alpha=0.0, gamma=gamma, epsilon=0.0
        ),
    )
...
class MAPPolicy:
    """Paper Eq. 21 with model-tie and action-tie rules kept separate.

    The paper defines the MAP model through an ``argmax`` over the posterior and
    then acts greedily with the selected model-specific Q-function, but it does
    not specify either tie convention.  Our Figure 8/9 exact-Q diagnostics show
    that the paper curves are consistent with:

    1. selecting the *first* model when several models share the maximum
       posterior probability; and
    2. selecting uniformly at random among *exactly tied* greedy actions.

    Other model-tie rules are retained for sensitivity analysis.
    """
    ...
    def __init__(
        ...
        base_seed: int,
        tie_rule: str = "first",
        action_tie_rule: str = "random",
    ) -> None:
        if tie_rule not in {"first", "sticky-random", "random-each-step"}:
            raise ValueError(
                "MAP model tie rule must be first, sticky-random, or random-each-step."
            )
        if action_tie_rule not in {"first", "random"}:
            raise ValueError("MAP action tie rule must be first or random.")
    ...
    def _action_index(self, q_values: np.ndarray) -> int:
        if self.action_tie_rule == "first":
            return int(np.argmax(q_values))
        return random_argmax(np.asarray(q_values, dtype=np.float64), self.rng)

    def select_action(self, state: InjurySearchState) -> int:
        model_index = self._model_index(state.model_probabilities)
        q_values = self.bank.values_for_model(model_index, state)
        return self._action_index(q_values)
...
def print_audit(scenario: NavigationScenario) -> None:
    ...
    print("- MAP ties are split into model/action rules; reproduction uses first/random")
    ...
    print("- The paper does not publish input encoding, MAP model/action tie rules, or seeds")
    print()
...
def diagnose_policy_values(
    *,
    ...
    map_policy = MAPPolicy(
        comparator_bank,
        map_tie_seed,
        tie_rule=map_tie_rule,
        action_tie_rule=map_action_tie_rule,
    )
    ...
    print(f"map_model_tie_rule={map_tie_rule}")
    print(f"map_action_tie_rule={map_action_tie_rule}")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default="first",
        help=(
            "Equation (21) posterior/model tie convention. The Figure 8/9 "
            "reproduction default is first."
        ),
    )
    parser.add_argument(
        "--map-action-tie-rule",
        ...
        default="random",
        help=(
            "Greedy action tie convention inside the selected MAP model. "
            "The Figure 8/9 reproduction default is uniform random among "
            "exactly tied maxima."
        ),
    )
    ...
    parser.add_argument(
        "--map-q-backend",
        choices=("exact", "comparator"),
        default="exact",
        help=(
            "Q source used only by MAP. exact computes model-specific Q* by "
            "value iteration without retraining; comparator reuses the supplied "
            "DQN/tabular comparator bank. Baseline and Active always use the "
            "supplied comparator bank."
        ),
    )
...
def main() -> None:
    ...
    if args.mode in {"diagnose-policies", "evaluate-all"}:
        ...
        map_bank = (
            build_exact_comparator_bank(scenario, gamma=0.95)
            if args.map_q_backend == "exact"
            else bank
        )
        ...
        if args.mode == "diagnose-policies":
            diagnose_policy_values(
                ...
                map_action_tie_rule=args.map_action_tie_rule,
        ...
        results = {
            ...
            "MAP Policy": evaluate_policy(
                ...
                MAPPolicy(
                    ...
                    action_tie_rule=args.map_action_tie_rule,
                ),
                args.trials,
                args.eval_horizon,
                args.base_eval_seed,
        ...
        payload: dict[str, np.ndarray] = {
            ...
            "map_action_tie_rule": np.asarray(args.map_action_tie_rule),
            "map_q_backend": np.asarray(args.map_q_backend),
        }
